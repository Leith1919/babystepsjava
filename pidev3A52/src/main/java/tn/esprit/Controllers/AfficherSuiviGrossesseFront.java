...
public class AfficherSuiviGrossesseFront implements Initializable {
    ...
    @FXML
    private Button btnGestionArticle;

    @FXML
    private AnchorPane contentArea;
    // ID spécifique à afficher
    private final int SUIVI_GROSSESSE_ID = 27;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        suiviGrossesseService = new SuiviGrossesseService();
        btnGestionArticle.setOnAction(e -> loadGestionArticle());
    ...
    private void loadGestionArticle() {
        try {
            AnchorPane pane = FXMLLoader.load(getClass().getResource("/Articles/Front/Article_Front.fxml"));
