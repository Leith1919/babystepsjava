...
public class InscriptionUser implements Initializable {
    ...
    void inscription(ActionEvent event) throws SQLException, IOException {
        ...
        String numtelStr = numtelTF.getText().trim();

// Validate phone number input before parsing
        if (numtelStr.isEmpty()) {
            showAlert("Phone number field cannot be empty");
            return;
        }

        if (!numtelStr.matches("\\d+")) {
            showAlert("Phone number must contain only digits");
            return;
        }

        if (numtelStr.length() < 8 || numtelStr.length() > 15) {
            showAlert("Phone number must be between 8 and 15 digits");
            return;
        }

        int numtel;
        try {
            numtel = Integer.parseInt(numtelStr);
