...
import services.User.AnnulationRendezVousService;
import services.User.AnnulationRendezVousService.ResultatAnnulation;
...
public class AfficherRendezVousFront {
    ...
    private void initialize() throws SQLException {
        ...
        if (currentUser == null) {
            showAlert(Alert.AlertType.ERROR, "Erreur de session",
                    "Aucun utilisateur connecté. Veuillez vous connecter pour voir vos rendez-vous.");
            return;
        }
    ...
    private void rafraichirCards() throws SQLException {
        ...
        List<RendezVous> rendezVousList = rendezVousService.afficherParPatient(currentUser.getId());
        cardsContainer.getChildren().clear();
    ...
    private void setupFilterEvents() {
        ...
        btnToday.setOnAction(e -> {
            try {
                ...
                List<RendezVous> todayList = rendezVousService.afficherParPatient(currentUser.getId()).stream()
        ...
        btnWeek.setOnAction(e -> {
            try {
                ...
                List<RendezVous> weekList = rendezVousService.afficherParPatient(currentUser.getId()).stream()
    ...
    private void filtrerCardParRecherche(String searchText) throws SQLException {
        ...
        List<RendezVous> allRendezVous = rendezVousService.afficherParPatient(currentUser.getId());
