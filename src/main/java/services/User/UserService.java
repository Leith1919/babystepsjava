...
public class UserService implements IServices<User> {
    ...
    public void ajouterGoogleUser(User user) throws SQLException {
        String req = "INSERT INTO user(nom, prenom, nationnalite, email, password, roles, numtel) VALUES(?, ?, ?, ?, ?, ?, ?)";
        PreparedStatement st = connection.prepareStatement(req);
        st.setString(1, user.getNom() != null ? user.getNom() : "GoogleUser");
        st.setString(2, user.getPrenom() != null ? user.getPrenom() : "User");
        st.setString(3, user.getNationnalite() != null ? user.getNationnalite() : "Tunisian");
        st.setString(4, user.getEmail().trim().toLowerCase());
        st.setString(5, "temporary");  // Pas de mot de passe pour Google
        st.setString(6, user.getRoles());
        st.setInt(7, 0);  // Numéro par défaut
        st.executeUpdate();
    }
   ...
   public void editProfile(User user) throws SQLException {
    ...
    public int countBannedUsers() throws SQLException {
        String query = "SELECT COUNT(*) FROM user WHERE banned = true";
        try (PreparedStatement ps = connection.prepareStatement(query);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1);
    ...
    public int countAdminUsers() throws SQLException {
        String query = "SELECT COUNT(*) FROM user WHERE roles LIKE '%ROLE_ADMIN%'";
        try (PreparedStatement ps = connection.prepareStatement(query);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
                return rs.getInt(1);
    ...
    public User findByEmail(String email) {
        // Implémentation basique, adapte-la selon ton accès à la base
        String query = "SELECT * FROM user WHERE email = ?";
        try {
